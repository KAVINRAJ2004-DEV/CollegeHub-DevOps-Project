            ...
            steps {
                withCredentials([file(credentialsId: 'collegehub-aws-key', variable: 'SSH_KEY')]) {
                    bat '''
                        ...
                        ssh -i "%SSH_KEY%" -o StrictHostKeyChecking=no ec2-user@15.252.173.68 "ssh -i ~/CollegeHub-Key.pem -o StrictHostKeyChecking=no ec2-user@10.0.4.18 \\"cd ~/CollegeHub-DevOps-Project && git pull && cd backend && sudo docker build -t collegehub-backend:latest . && sudo docker rm -f collegehub-backend && sudo docker run -d --name collegehub-backend -p 3000:3000 collegehub-backend:latest\\""
                    '''
                }
            }
        }
